...
//
#include "robot_three.h"
#include "robot_three_private.h"
#define robot_three_MessageQueueLen    (105)
#define robot_three_MessageQueueLen_h  (1)
...
//    '<S3>/Conversion2'
//
void robot_three_Conversion1(real_T rtu_x, real_T rtu_y, real_T rtu_z, real_T
  rtu_w, B_Conversion1_robot_three_T *localB)
{
  real_T b_y;
  real_T q_idx_0;
  real_T q_idx_1;
...
void robot_0ModelClass::step()
{
  ...
  varargout_1 = Sub_robot_three_11.getLatestMessage(&robot_three_B.varargout_2);

  // Outputs for Enabled SubSystem: '<S14>/Enabled Subsystem' incorporates:
  //   EnablePort: '<S16>/Enable'

  if (varargout_1) {
    robot_three_B.In1 = robot_three_B.varargout_2;
  }
  ...
  varargout_1_0 = Sub_robot_three_10.getLatestMessage(&robot_three_B.varargout_2);

  // Outputs for Enabled SubSystem: '<S13>/Enabled Subsystem' incorporates:
  //   EnablePort: '<S15>/Enable'

  if (varargout_1_0) {
    robot_three_B.In1_d = robot_three_B.varargout_2;
  }
  ...
  // '<S4>:1:4' y1 = ((1-0.2)*u1+(u3*0.2)+(1.5*0.2));
  // '<S4>:1:5' y2 = ((1-0.2)*u2+(0.2*u4)-(0*0.2));
  // '<S4>:1:6' y3=u1;
  // '<S4>:1:7' y4=u2;
  robot_three_Conversion1(robot_three_B.In1.Pose.Pose.Orientation.X,
    robot_three_B.In1.Pose.Pose.Orientation.Y,
    robot_three_B.In1.Pose.Pose.Orientation.Z,
    robot_three_B.In1.Pose.Pose.Orientation.W, &robot_three_B.sf_Conversion2);
  ...
  if (varargout_1) {
    ...
    robot_three_B.Sum1 = (0.8 * robot_three_B.In1.Pose.Pose.Position.Y + 0.2 *
                          robot_three_B.In1_d.Pose.Pose.Position.Y) -
      robot_three_B.In1.Pose.Pose.Position.Y;
    ...
    robot_three_B.Sum = ((0.8 * robot_three_B.In1.Pose.Pose.Position.X +
                          robot_three_B.In1_d.Pose.Pose.Position.X * 0.2) +
                         0.30000000000000004) -
      robot_three_B.In1.Pose.Pose.Position.X;

    // Fcn: '<S9>/Distance1'
    robot_three_B.Sum3 = rt_powd_snf(robot_three_B.Sum, 2.0) + rt_powd_snf
      (robot_three_B.Sum1, 2.0);
    if (robot_three_B.Sum3 < 0.0) {
      robot_three_B.Sum3 = -sqrt(-robot_three_B.Sum3);
    } else {
      robot_three_B.Sum3 = sqrt(robot_three_B.Sum3);
    }
    ...
    if (robot_three_B.Sum3 <= robot_three_P.speedThreshold2_const) {
      robot_three_B.Switch1 = robot_three_P.Stop1_Value;
      robot_three_B.Sum1 = robot_three_P.Stop1_Value;
    } else {
      // Sum: '<S9>/Sum3' incorporates:
      //   Trigonometry: '<S9>/Desired Yaw'

      robot_three_B.Sum3 = rt_atan2d_snf(robot_three_B.Sum1, robot_three_B.Sum)
        - robot_three_B.sf_Conversion2.yaw;
      robot_three_B.Switch1 = rt_atan2d_snf(sin(robot_three_B.Sum3), cos
        (robot_three_B.Sum3));

      // Fcn: '<S9>/Distance3' incorporates:
      //   Fcn: '<S9>/Bound [-pi,pi]'

      robot_three_B.Sum1 *= 5.0;

      // Fcn: '<S9>/Distance2'
      robot_three_B.Sum *= 5.0;

      // Fcn: '<S9>/Distance'
      robot_three_B.Sum3 = rt_powd_snf(robot_three_B.Sum, 2.0) + rt_powd_snf
        (robot_three_B.Sum1, 2.0);
      if (robot_three_B.Sum3 < 0.0) {
        robot_three_B.Sum1 = -sqrt(-robot_three_B.Sum3);
      } else {
        robot_three_B.Sum1 = sqrt(robot_three_B.Sum3);
      }
    ...
    robot_three_B.BusAssignment1 = robot_three_P.Constant_Value_p;
    robot_three_B.BusAssignment1.Linear.X =
      robot_three_P.LinearVelocitySlider_gain * robot_three_B.Sum1;
    robot_three_B.BusAssignment1.Angular.Z = robot_three_B.Switch1;

    // Outputs for Atomic SubSystem: '<S5>/Publish2'
    // Start for MATLABSystem: '<S8>/SinkBlock' incorporates:
    //   MATLABSystem: '<S8>/SinkBlock'

    Pub_robot_three_75.publish(&robot_three_B.BusAssignment1);
  ...
  robot_three_Conversion1(robot_three_B.In1_d.Pose.Pose.Orientation.X,
    robot_three_B.In1_d.Pose.Pose.Orientation.Y,
    robot_three_B.In1_d.Pose.Pose.Orientation.Z,
    robot_three_B.In1_d.Pose.Pose.Orientation.W, &robot_three_B.sf_Conversion1);
}
...
void robot_0ModelClass::initialize()
{
  ...
  rtmSetErrorStatus((&robot_three_M), (NULL));

  // block I/O
  (void) memset(((void *) &robot_three_B), 0,
                sizeof(B_robot_three_T));

  // states (dwork)
  (void) memset((void *)&robot_three_DW, 0,
                sizeof(DW_robot_three_T));

  {
    ...
    robot_three_DW.obj_o.isInitialized = 0;
    robot_three_DW.obj_o.isInitialized = 1;
    for (i = 0; i < 13; i++) {
      tmp_3[i] = tmp_1[i];
    }

    tmp_3[13] = '\x00';
    Sub_robot_three_11.createSubscriber(tmp_3, robot_three_MessageQueueLen_h);
    ...
    robot_three_DW.obj_n.isInitialized = 0;
    robot_three_DW.obj_n.isInitialized = 1;
    for (i = 0; i < 13; i++) {
      tmp_3[i] = tmp_0[i];
    }

    tmp_3[13] = '\x00';
    Sub_robot_three_10.createSubscriber(tmp_3, robot_three_MessageQueueLen_h);
    ...
    robot_three_DW.obj.isInitialized = 0;
    robot_three_DW.obj.isInitialized = 1;
    for (i = 0; i < 16; i++) {
      tmp_2[i] = tmp[i];
    }

    tmp_2[16] = '\x00';
    Pub_robot_three_75.createPublisher(tmp_2, robot_three_MessageQueueLen);
    ...
    robot_three_B.In1 = robot_three_P.Out1_Y0_p;
    ...
    robot_three_B.In1_d = robot_three_P.Out1_Y0;
...
void robot_0ModelClass::terminate()
{
  ...
  if (robot_three_DW.obj_o.isInitialized == 1) {
    robot_three_DW.obj_o.isInitialized = 2;
  }
  ...
  if (robot_three_DW.obj_n.isInitialized == 1) {
    robot_three_DW.obj_n.isInitialized = 2;
  }
  ...
  if (robot_three_DW.obj.isInitialized == 1) {
    robot_three_DW.obj.isInitialized = 2;
  }
...
{
  static const P_robot_three_T robot_three_P_temp = {
  ...
  robot_three_P = robot_three_P_temp;
}
...
RT_MODEL_robot_three_T * robot_0ModelClass::getRTM()
{
  return (&robot_three_M);
}
