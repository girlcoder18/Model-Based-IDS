...
// File: robot_cam2.cpp
//
// Code generated for Simulink model 'robot_cam2'.
//
// Model version                  : 1.20
// Simulink Coder version         : 8.11 (R2016b) 25-Aug-2016
// C/C++ source code generated on : Wed Feb  1 20:24:58 2017
//
...
//
#include "robot_cam2.h"
#include "robot_cam2_private.h"
#define robot_cam2_MessageQueueLen     (105)
#define robot_cam2_MessageQueueLen_o   (1)
...
//    '<S3>/Conversion2'
//
void robot_cam2_Conversion1(real_T rtu_x, real_T rtu_y, real_T rtu_z, real_T
  rtu_w, B_Conversion1_robot_cam2_T *localB)
{
  real_T b_y;
  real_T q_idx_0;
  real_T q_idx_1;
...
void robot_0ModelClass::step()
{
  ...
  varargout_1 = Sub_robot_cam2_11.getLatestMessage(&robot_cam2_B.varargout_2);

  // Outputs for Enabled SubSystem: '<S14>/Enabled Subsystem' incorporates:
  //   EnablePort: '<S16>/Enable'

  if (varargout_1) {
    robot_cam2_B.In1 = robot_cam2_B.varargout_2;
  }
  ...
  varargout_1_0 = Sub_robot_cam2_10.getLatestMessage(&robot_cam2_B.varargout_2);

  // Outputs for Enabled SubSystem: '<S13>/Enabled Subsystem' incorporates:
  //   EnablePort: '<S15>/Enable'

  if (varargout_1_0) {
    robot_cam2_B.In1_d = robot_cam2_B.varargout_2;
  }
  ...
  robot_cam2_Conversion1(robot_cam2_B.In1.Pose.Orientation.X,
    robot_cam2_B.In1.Pose.Orientation.Y, robot_cam2_B.In1.Pose.Orientation.Z,
    robot_cam2_B.In1.Pose.Orientation.W, &robot_cam2_B.sf_Conversion2);
  ...
  if (varargout_1) {
    ...
    robot_cam2_B.Sum1 = ((0.8 * robot_cam2_B.In1.Pose.Position.Y + 0.2 *
                          robot_cam2_B.In1_d.Pose.Position.Y) - 0.2598) -
      robot_cam2_B.In1.Pose.Position.Y;
    ...
    robot_cam2_B.Sum = ((0.8 * robot_cam2_B.In1.Pose.Position.X +
                         robot_cam2_B.In1_d.Pose.Position.X * 0.2) -
                        0.15000000000000002) - robot_cam2_B.In1.Pose.Position.X;

    // Fcn: '<S9>/Distance1'
    robot_cam2_B.Sum3 = rt_powd_snf(robot_cam2_B.Sum, 2.0) + rt_powd_snf
      (robot_cam2_B.Sum1, 2.0);
    if (robot_cam2_B.Sum3 < 0.0) {
      robot_cam2_B.Sum3 = -sqrt(-robot_cam2_B.Sum3);
    } else {
      robot_cam2_B.Sum3 = sqrt(robot_cam2_B.Sum3);
    }
    ...
    if (robot_cam2_B.Sum3 <= robot_cam2_P.speedThreshold2_const) {
      robot_cam2_B.Switch1 = robot_cam2_P.Stop1_Value;
      robot_cam2_B.Sum1 = robot_cam2_P.Stop1_Value;
    } else {
      // Sum: '<S9>/Sum3' incorporates:
      //   Trigonometry: '<S9>/Desired Yaw'

      robot_cam2_B.Sum3 = rt_atan2d_snf(robot_cam2_B.Sum1, robot_cam2_B.Sum) -
        robot_cam2_B.sf_Conversion2.yaw;
      robot_cam2_B.Switch1 = rt_atan2d_snf(sin(robot_cam2_B.Sum3), cos
        (robot_cam2_B.Sum3));

      // Fcn: '<S9>/Distance3' incorporates:
      //   Fcn: '<S9>/Bound [-pi,pi]'

      robot_cam2_B.Sum1 *= 5.0;

      // Fcn: '<S9>/Distance2'
      robot_cam2_B.Sum *= 5.0;

      // Fcn: '<S9>/Distance'
      robot_cam2_B.Sum3 = rt_powd_snf(robot_cam2_B.Sum, 2.0) + rt_powd_snf
        (robot_cam2_B.Sum1, 2.0);
      if (robot_cam2_B.Sum3 < 0.0) {
        robot_cam2_B.Sum1 = -sqrt(-robot_cam2_B.Sum3);
      } else {
        robot_cam2_B.Sum1 = sqrt(robot_cam2_B.Sum3);
      }
    ...
    robot_cam2_B.BusAssignment1 = robot_cam2_P.Constant_Value_p;
    robot_cam2_B.BusAssignment1.Linear.X =
      robot_cam2_P.LinearVelocitySlider_gain * robot_cam2_B.Sum1;
    robot_cam2_B.BusAssignment1.Angular.Z = robot_cam2_B.Switch1;

    // Outputs for Atomic SubSystem: '<S5>/Publish2'
    // Start for MATLABSystem: '<S8>/SinkBlock' incorporates:
    //   MATLABSystem: '<S8>/SinkBlock'

    Pub_robot_cam2_75.publish(&robot_cam2_B.BusAssignment1);
  ...
  robot_cam2_Conversion1(robot_cam2_B.In1_d.Pose.Orientation.X,
    robot_cam2_B.In1_d.Pose.Orientation.Y, robot_cam2_B.In1_d.Pose.Orientation.Z,
    robot_cam2_B.In1_d.Pose.Orientation.W, &robot_cam2_B.sf_Conversion1);
}
...
void robot_0ModelClass::initialize()
{
  ...
  rtmSetErrorStatus((&robot_cam2_M), (NULL));

  // block I/O
  (void) memset(((void *) &robot_cam2_B), 0,
                sizeof(B_robot_cam2_T));

  // states (dwork)
  (void) memset((void *)&robot_cam2_DW, 0,
                sizeof(DW_robot_cam2_T));

  {
    static const char_T tmp[16] = { '/', 'r', 'o', 'b', 'o', 't', '_', '1', '/',
      'c', 'm', 'd', '_', 'v', 'e', 'l' };

    static const char_T tmp_0[11] = { '/', 'b', 'o', 't', '_', '3', '/', 'p',
      'o', 's', 'e' };

    static const char_T tmp_1[11] = { '/', 'b', 'o', 't', '_', '1', '/', 'p',
      'o', 's', 'e' };
    ...
    robot_cam2_DW.obj_o.isInitialized = 0;
    robot_cam2_DW.obj_o.isInitialized = 1;
    for (i = 0; i < 11; i++) {
      tmp_3[i] = tmp_1[i];
    }

    tmp_3[11] = '\x00';
    Sub_robot_cam2_11.createSubscriber(tmp_3, robot_cam2_MessageQueueLen_o);
    ...
    robot_cam2_DW.obj_n.isInitialized = 0;
    robot_cam2_DW.obj_n.isInitialized = 1;
    for (i = 0; i < 11; i++) {
      tmp_3[i] = tmp_0[i];
    }

    tmp_3[11] = '\x00';
    Sub_robot_cam2_10.createSubscriber(tmp_3, robot_cam2_MessageQueueLen_o);
    ...
    robot_cam2_DW.obj.isInitialized = 0;
    robot_cam2_DW.obj.isInitialized = 1;
    for (i = 0; i < 16; i++) {
      tmp_2[i] = tmp[i];
    }

    tmp_2[16] = '\x00';
    Pub_robot_cam2_75.createPublisher(tmp_2, robot_cam2_MessageQueueLen);
    ...
    robot_cam2_B.In1 = robot_cam2_P.Out1_Y0_p;
    ...
    robot_cam2_B.In1_d = robot_cam2_P.Out1_Y0;
...
void robot_0ModelClass::terminate()
{
  ...
  if (robot_cam2_DW.obj_o.isInitialized == 1) {
    robot_cam2_DW.obj_o.isInitialized = 2;
  }
  ...
  if (robot_cam2_DW.obj_n.isInitialized == 1) {
    robot_cam2_DW.obj_n.isInitialized = 2;
  }
  ...
  if (robot_cam2_DW.obj.isInitialized == 1) {
    robot_cam2_DW.obj.isInitialized = 2;
  }
...
{
  static const P_robot_cam2_T robot_cam2_P_temp = {
  ...
  robot_cam2_P = robot_cam2_P_temp;
}
...
RT_MODEL_robot_cam2_T * robot_0ModelClass::getRTM()
{
  return (&robot_cam2_M);
}
