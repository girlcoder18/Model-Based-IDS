...
//
#include "robot_cam2a.h"
#include "robot_cam2a_private.h"
#define robot_cam2a_MessageQueueLen    (105)
#define robot_cam2a_MessageQueueLen_e  (1)
...
void robot_0ModelClass::step()
{
  ...
  varargout_1 = Sub_robot_cam2a_78.getLatestMessage(&robot_cam2a_B.varargout_2);
  ...
  if (varargout_1) {
    ...
    rtb_Sum1 = ((0.8 * robot_cam2a_B.UDPReceive_o1[4] + 0.2 *
                 robot_cam2a_B.UDPReceive_o1[7]) - 0.2598) -
      robot_cam2a_B.UDPReceive_o1[4];

    // Sum: '<S9>/Sum' incorporates:
    //   MATLAB Function: '<Root>/cyc_pursuit2'

    rtb_Sum = ((0.8 * robot_cam2a_B.UDPReceive_o1[3] +
                robot_cam2a_B.UDPReceive_o1[6] * 0.2) - 0.15000000000000002) -
      robot_cam2a_B.UDPReceive_o1[3];
    ...
    if (rtb_Sum3 <= robot_cam2a_P.speedThreshold2_const) {
      Switch1 = robot_cam2a_P.Stop1_Value;
      rtb_Sum1 = robot_cam2a_P.Stop1_Value;
    } else {
      // Sum: '<S9>/Sum3' incorporates:
      //   Trigonometry: '<S9>/Desired Yaw'

      rtb_Sum3 = rt_atan2d_snf(rtb_Sum1, rtb_Sum) - robot_cam2a_B.UDPReceive_o1
        [5];
    ...
    robot_cam2a_B.BusAssignment1 = robot_cam2a_P.Constant_Value_p;
    robot_cam2a_B.BusAssignment1.Linear.X =
      robot_cam2a_P.LinearVelocitySlider_gain * rtb_Sum1;
    robot_cam2a_B.BusAssignment1.Angular.Z = Switch1;

    // Outputs for Atomic SubSystem: '<S5>/Publish2'
    // Start for MATLABSystem: '<S8>/SinkBlock' incorporates:
    //   MATLABSystem: '<S8>/SinkBlock'

    Pub_robot_cam2a_75.publish(&robot_cam2a_B.BusAssignment1);
...
void robot_0ModelClass::initialize()
{
  ...
  rtmSetErrorStatus((&robot_cam2a_M), (NULL));

  // states (dwork)
  (void) memset((void *)&robot_cam2a_DW, 0,
                sizeof(DW_robot_cam2a_T));

  {
    ...
    robot_cam2a_DW.obj_c.isInitialized = 0;
    robot_cam2a_DW.obj_c.isInitialized = 1;
    for (i = 0; i < 13; i++) {
      tmp_2[i] = tmp_0[i];
    }

    tmp_2[13] = '\x00';
    Sub_robot_cam2a_78.createSubscriber(tmp_2, robot_cam2a_MessageQueueLen_e);
    ...
    robot_cam2a_DW.obj.isInitialized = 0;
    robot_cam2a_DW.obj.isInitialized = 1;
    for (i = 0; i < 16; i++) {
      tmp_1[i] = tmp[i];
    }

    tmp_1[16] = '\x00';
    Pub_robot_cam2a_75.createPublisher(tmp_1, robot_cam2a_MessageQueueLen);
...
void robot_0ModelClass::terminate()
{
  ...
  if (robot_cam2a_DW.obj_c.isInitialized == 1) {
    robot_cam2a_DW.obj_c.isInitialized = 2;
  }
  ...
  if (robot_cam2a_DW.obj.isInitialized == 1) {
    robot_cam2a_DW.obj.isInitialized = 2;
  }
...
{
  static const P_robot_cam2a_T robot_cam2a_P_temp = {
  ...
  robot_cam2a_P = robot_cam2a_P_temp;
}
...
RT_MODEL_robot_cam2a_T * robot_0ModelClass::getRTM()
{
  return (&robot_cam2a_M);
}
