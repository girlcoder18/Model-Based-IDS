...
//
#include "robot_cam3a.h"
#include "robot_cam3a_private.h"
#define robot_cam3a_MessageQueueLen    (105)
#define robot_cam3a_MessageQueueLen_e  (1)
...
void robot_0ModelClass::step()
{
  ...
  // '<S4>:1:6' y3=u1;
  // '<S4>:1:7' y4=u2;
  varargout_1 = Sub_robot_cam3a_78.getLatestMessage(&robot_cam3a_B.varargout_2);
  ...
  if (varargout_1) {
    ...
    rtb_Sum1 = (0.8 * robot_cam3a_B.UDPReceive_o1[7] + 0.2 *
                robot_cam3a_B.UDPReceive_o1[1]) - robot_cam3a_B.UDPReceive_o1[7];

    // Sum: '<S9>/Sum' incorporates:
    //   MATLAB Function: '<Root>/cyc_pursuit2'

    rtb_Sum = ((0.8 * robot_cam3a_B.UDPReceive_o1[6] +
                robot_cam3a_B.UDPReceive_o1[0] * 0.2) + 0.30000000000000004) -
      robot_cam3a_B.UDPReceive_o1[6];
    ...
    if (rtb_Sum3 <= robot_cam3a_P.speedThreshold2_const) {
      Switch1 = robot_cam3a_P.Stop1_Value;
      rtb_Sum1 = robot_cam3a_P.Stop1_Value;
    } else {
      // Sum: '<S9>/Sum3' incorporates:
      //   Trigonometry: '<S9>/Desired Yaw'

      rtb_Sum3 = rt_atan2d_snf(rtb_Sum1, rtb_Sum) - robot_cam3a_B.UDPReceive_o1
        [8];
    ...
    robot_cam3a_B.BusAssignment1 = robot_cam3a_P.Constant_Value_p;
    robot_cam3a_B.BusAssignment1.Linear.X =
      robot_cam3a_P.LinearVelocitySlider_gain * rtb_Sum1;
    robot_cam3a_B.BusAssignment1.Angular.Z = Switch1;

    // Outputs for Atomic SubSystem: '<S5>/Publish2'
    // Start for MATLABSystem: '<S8>/SinkBlock' incorporates:
    //   MATLABSystem: '<S8>/SinkBlock'

    Pub_robot_cam3a_75.publish(&robot_cam3a_B.BusAssignment1);
...
void robot_0ModelClass::initialize()
{
  ...
  rtmSetErrorStatus((&robot_cam3a_M), (NULL));

  // states (dwork)
  (void) memset((void *)&robot_cam3a_DW, 0,
                sizeof(DW_robot_cam3a_T));

  {
    ...
    robot_cam3a_DW.obj_c.isInitialized = 0;
    robot_cam3a_DW.obj_c.isInitialized = 1;
    for (i = 0; i < 13; i++) {
      tmp_2[i] = tmp_0[i];
    }

    tmp_2[13] = '\x00';
    Sub_robot_cam3a_78.createSubscriber(tmp_2, robot_cam3a_MessageQueueLen_e);
    ...
    robot_cam3a_DW.obj.isInitialized = 0;
    robot_cam3a_DW.obj.isInitialized = 1;
    for (i = 0; i < 16; i++) {
      tmp_1[i] = tmp[i];
    }

    tmp_1[16] = '\x00';
    Pub_robot_cam3a_75.createPublisher(tmp_1, robot_cam3a_MessageQueueLen);
...
void robot_0ModelClass::terminate()
{
  ...
  if (robot_cam3a_DW.obj_c.isInitialized == 1) {
    robot_cam3a_DW.obj_c.isInitialized = 2;
  }
  ...
  if (robot_cam3a_DW.obj.isInitialized == 1) {
    robot_cam3a_DW.obj.isInitialized = 2;
  }
...
{
  static const P_robot_cam3a_T robot_cam3a_P_temp = {
  ...
  robot_cam3a_P = robot_cam3a_P_temp;
}
...
RT_MODEL_robot_cam3a_T * robot_0ModelClass::getRTM()
{
  return (&robot_cam3a_M);
}
