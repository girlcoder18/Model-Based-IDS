...
// C/C++ source code generated on : Fri Dec 30 17:24:27 2016
//
...
void robot_0ModelClass::step()
{
  ...
  // '<S4>:1:4' y1 = ((1-0.2)*u1+(u3*0.2)-(1.2135*0.2));
  // '<S4>:1:5' y2 = ((1-0.2)*u2+(0.2*u4)+(0.8817*0.2));
  // '<S4>:1:6' y3=u1;
  // '<S4>:1:7' y4=u2;
  ...
  if (varargout_1) {
    ...
    robot_B.Sum1 = ((0.8 * robot_B.In1.Pose.Position.Y + 0.2 *
                     robot_B.In1_d.Pose.Position.Y) + 0.17634000000000002) -
    ...
    robot_B.Sum = ((0.8 * robot_B.In1.Pose.Position.X +
                    robot_B.In1_d.Pose.Position.X * 0.2) - 0.24270000000000003)
